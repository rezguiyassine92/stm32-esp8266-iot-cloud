...
/* Private define ------------------------------------------------------------*/
/* USER CODE BEGIN PD */
#define __CIPSTART "AT+CIPSTART=0,\"TCP\",\"api.thingspeak.com\",80\r\n"

#define WIFI_NAME     "TOPNET_5ED0"
#define WIFI_PASSWORD "191919982761198"

// Commandes AT adaptées pour ESP32
#define __RST         "AT+RST\r\n"
#define __AT          "AT\r\n"
...
uint16_t ADC_Results[3]; // Tableau pour stocker les 3 valeurs (PC0, PC1, PC2)
...
void ESP8266_Init (void)
{
  ...
  // Utilisation directe des macros
...
void ESP8266_SendServer (void)
{
  // 1. On lance la connexion au serveur
  ESP8266_SendCommand (__CIPSTART);
  HAL_Delay(500); // Petite pause nécessaire pour que la connexion s'établisse

  // 2. On prépare le message pour ThingSpeak
  memset(BUFFER_TO_SEND, '\0', sizeof(BUFFER_TO_SEND));
  sprintf(BUFFER_TO_SEND, "GET /update?api_key=H26UVWP20AEFY28B&field1=%d&field2=%d&field3=%d\r\n",
          ADC_Results[0], ADC_Results[1], ADC_Results[2]);

  // 3. On dit à l'ESP combien de caractères on va envoyer
  sprintf((char*)__CIPSEND, "AT+CIPSEND=0,%d\r\n", strlen(BUFFER_TO_SEND));
  ESP8266_SendCommand (__CIPSEND);

  // 4. On envoie les données
  ESP8266_SendCommand((uint8_t*)BUFFER_TO_SEND);

  // 5. On ferme la connexion
  ESP8266_SendCommand("AT+CIPCLOSE=0\r\n");

  Exti0 = 0;
  HAL_Delay(15000); // Attendre 15 secondes (limite de ThingSpeak)
}
...
void ESP8266_SendCommand (uint8_t * command)
{
  ...
  ESP8266_Receive();
  HAL_UART_Transmit(&huart2, command, strlen((const char*)command), 1000);

  while (UART_DMA_DataIsReady == 0 && timeout > 0) {
      timeout--;
  }
  HAL_Delay(50);
}
